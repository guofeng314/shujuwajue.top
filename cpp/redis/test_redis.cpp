...
int main()
{
	...
	GFRedis redis("10.119.135.62", 51214);
 	vector<string> ks;
	ks.push_back("name");
	ks.push_back("name2");	
	...
	Json::Value json = redis.GetAdData("aaa", "DATA", "FIRST", 3);
	Json::Value add_value = json["result"];
	for (unsigned int i = 0; i < add_value.size(); ++i)
        {
            Json::Value temp_value = add_value[i];
	    cout<<"res"<<i<<":"<<temp_value["url"].asString()<<endl;
	}
}
